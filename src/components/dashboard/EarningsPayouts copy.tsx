...
export default function EarningsPayouts() {
  ...
  const parseDate = (dateStr: string) => parse(dateStr, 'MMM d, yyyy', new Date());

  const filteredEarnings = selectedRange
  ...
  const filteredPayouts = selectedRange
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
          ...
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" onClick={() => setCalendarOpen(!calendarOpen)}>
                ...
                {selectedRange ? `${format(selectedRange.from, 'PPP')} - ${format(selectedRange.to, 'PPP')}` : 'Select Date Range'}
              </Button>
            </PopoverTrigger>
      ...
      <div className="bg-white rounded-lg border">
        ...
        <div className="flex justify-between items-center p-4 text-sm text-gray-600">
          <span>Showing 7 of 120 earnings</span>
